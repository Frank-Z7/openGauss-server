...
bool PageCompress::CompressOnePage(void)
{
    ...
    for (row = 0; row < m_mappedTupsNum; ++row) {
         ...
         */
        if (hascmpr) {
            fillOnePage = Dispatch(m_mappedTups[row], (HeapTuple)tableam_tops_form_cmprs_tuple(desc, &formTupleData), size, blksize);
        } else {
    ...
    for (row = m_current + m_mappedTupsNum + (m_toastTupsNum - prevToastNum); row < m_inTupsNum; ++row) {
        ...
        if (hascmpr) {
            fillOnePage = Dispatch(raw, (HeapTuple)tableam_tops_form_cmprs_tuple(desc, &formTupleData), size, blksize);
        } else {
