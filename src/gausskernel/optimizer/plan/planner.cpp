...
#define DEBUG_QRW(message)                                                                      \
    ...
    if (parse->resultRelation && parse->commandType != CMD_INSERT &&
        ...
        */
        if (parse->hasTargetSRFs && !IsA(plan, ModifyTable)) {
...
static Plan* grouping_planner(PlannerInfo* root, double tuple_fraction)
{
    ...
    MemoryContext oldcontext = NULL;
    errno_t rc = EOK;
    PlannerTargets* planner_targets = (PlannerTargets*)palloc0(sizeof(PlannerTargets));
    ...
    if (parse->setOperations) {
        ...
        if (parse->hasWindowFuncs) {
            ...
            if (wflists->numWindowFuncs > 0) {
                ...
                activeWindows = wflists->activeWindows;
            }
            else
        ...
        */
		...
		if (parse->hasTargetSRFs) {
            ...
            process_planner_targets(root, planner_targets, tlist, activeWindows); 
        ...
        if (result_plan != NULL) {
            ...
            if (need_tlist_eval) {
                 ...
                 * desired tlist.
                 */
                if (result_plan->type == T_ProjectSet) {
                    /*nothing to do*/
                }
    ...
    if (parse->distinctClause) {
        ...
        if (use_hashed_distinct) {
            ...
            if (IS_STREAM_PLAN && needs_stream && is_execute_on_datanodes(result_plan) &&
                !is_replicated_plan(result_plan)) {
                if (next_is_second_level_distinct && parse->hasTargetSRFs) {
                    errno_t sprintf_rc = sprintf_s(u_sess->opt_cxt.not_shipping_info->not_shipping_reason,
                        NOTPLANSHIPPING_LENGTH,
    ...
    Plan* result_plan, WindowLists* wflists, bool* need_hash, List* collectiveGroupExpr)
{
    ...
    if (root->planner_targets->grouping_contains_srfs) {
        stream_plan = false;
    }
