...
{
    Datum result;
    if (IsA(arg, SetExprState)) {
        /*
         * Evaluate SRF - possibly continuing previously started output.
         */
        result = ExecMakeFunctionResultSet((SetExprState*)arg, econtext, isnull, isdone);
		*hassrf = true;
    } else {
...
static TupleTableSlot *ExecProjectSRF(ProjectSetState *node)
{
    ...
    if (haveDoneSets) {
        ...
        for (argno = 0; argno < node->nelems; argno++) {
            ...
            *result = ExecMakeFunctionResultSet((SetExprState*)elem, econtext, isnull, itemIsDone);
