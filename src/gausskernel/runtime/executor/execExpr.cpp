...
static void ExecBuildAggTransCall(ExprState *state, AggState *aggstate,
					  ...
					  int transno, int setno, int setoff, bool ishash,bool iscollect);
						...
						TupleDesc inputDesc)
{
    ...
    projInfo->pi_trace_column_name = u_sess->attr.attr_sql.enable_trace_column;
	...
	foreach(lc, targetList)
	{
        ...
        if (unlikely(projInfo->pi_trace_column_name)) {
            scratch.d.trace_column.column_name = tle->resname;
            scratch.opcode = EEOP_TRACE_COLUMN;
            ExprEvalPushStep(state, &scratch);
        }
		...
		case T_Aggref:
			{
				...
				if (state->parent && (IsA(state->parent, AggState) || IsA(state->parent, VecAggState))) {
					...
					/*
					* XXX: reserve the code for vertor engine
					* we only need '++aggstate->numaggs;' after the commit 804163bc25e979fcd91b02e58fa2d1c6b587cc65
					* but since the vector engine does not adapt the commit
					* reserve the code for now
					*/
			 ...
			 Oid inputcollid, ExprState *state)
{
    ...
    if (!IsSystemObjOid(funcid) && supportTranaction && IsA(node, FuncExpr)) {
        ...
        if (!HeapTupleIsValid(tp)) {
            ereport(ERROR, (errcode(ERRCODE_UNDEFINED_OBJECT), errmsg("cache lookup failed for function %u", funcid)));
        }
...
ExecBuildAggTrans(AggState *aggstate, AggStatePerPhase phase, bool doSort, bool doHash)
{
    ...
    for (transno = 0; transno < aggstate->numtrans; transno++)
    {
        ...
        if (doSort)
        {
            ...
            {
                ExecBuildAggTransCall(state, aggstate, &scratch, isCollect?collect_fcinfo:trans_fcinfo,
                                            pertrans, transno, setno, setoff, false, isCollect);
                setoff++;
            }
        }
        ...
        if (doHash)
        {
			...
			{
				ExecBuildAggTransCall(state, aggstate, &scratch, isCollect?collect_fcinfo:trans_fcinfo,
									  pertrans, transno, setno, setoff, true, isCollect);
				setoff++;
			}
		}
    ...
    if (pertrans->numSortCols == 0)
    {
		if (pertrans->transtypeByVal)
		{
			if (fcinfo->flinfo->fn_strict &&
				...
				scratch->opcode = iscollect?EEOP_AGG_COLLECT_PLAIN_TRANS_INIT_STRICT_BYVAL:EEOP_AGG_PLAIN_TRANS_INIT_STRICT_BYVAL;
			else if (fcinfo->flinfo->fn_strict)
				scratch->opcode = iscollect?EEOP_AGG_COLLECT_PLAIN_TRANS_STRICT_BYVAL:EEOP_AGG_PLAIN_TRANS_STRICT_BYVAL;
			else
		...
		else
		{
			if (fcinfo->flinfo->fn_strict &&
				pertrans->initValueIsNull)
				scratch->opcode = iscollect?EEOP_AGG_COLLECT_PLAIN_TRANS_INIT_STRICT_BYREF:EEOP_AGG_PLAIN_TRANS_INIT_STRICT_BYREF;
			else if (fcinfo->flinfo->fn_strict)
				scratch->opcode = iscollect?EEOP_AGG_COLLECT_PLAIN_TRANS_STRICT_BYREF:EEOP_AGG_PLAIN_TRANS_STRICT_BYREF;
			else
				scratch->opcode = iscollect?EEOP_AGG_COLLECT_PLAIN_TRANS_BYREF:EEOP_AGG_PLAIN_TRANS_BYREF;
		}
	}
	else if (pertrans->numInputs == 1)
