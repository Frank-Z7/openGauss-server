...
ExecMakeFunctionResultNoSets(ExprState *state, ExprEvalStep *op,ExprContext *econtext)
{
	...
	*op->resnull = fcinfo->isnull;
    ...
    set_result_for_plpgsql_language_function_with_outparam(fcinfo->flinfo->fn_oid, op->resvalue, op->resnull);
...
ExecInterpExpr(ExprState *state, ExprContext *econtext, bool *isnull)
{
	...
	EEO_SWITCH()
	{
		...
		EEO_CASE(EEOP_INNER_VAR)
		{
			 ...
			 */
			Assert((innerslot->tts_tam_ops == TableAmHeap) ? (attnum >= 0 && attnum < innerslot->tts_nvalid) : (attnum >= 0));
		...
		EEO_CASE(EEOP_OUTER_VAR)
		{
			...
			Assert((outerslot->tts_tam_ops == TableAmHeap) ? (attnum >= 0 && attnum < outerslot->tts_nvalid) : (attnum >= 0));
		...
		EEO_CASE(EEOP_SCAN_VAR)
		{
			...
			Assert((scanslot->tts_tam_ops == TableAmHeap) ? (attnum >= 0 && attnum < scanslot->tts_nvalid) : (attnum >= 0));
...
ExecEvalHashFilter(ExprState *state, ExprEvalStep *op, ExprContext *econtext)
{
	...
	*op->resnull = true;
    ...
    forboth(distkey, op->d.hash_filter.arg, vartypes, op->d.hash_filter.typeOids)
    {
...
void
ExecAggInitGroup(AggState *aggstate, AggStatePerTrans pertrans, AggStatePerGroup pergroup, MemoryContext aggcontext)
{
...
void
ExecAggInitCollectGroup(AggState *aggstate, AggStatePerTrans pertrans, AggStatePerGroup pergroup, MemoryContext aggcontext)
{
    ...
    */
	oldContext = MemoryContextSwitchTo(aggcontext);
	pergroup->collectValue = datumCopy(fcinfo->arg[1],
									 pertrans->transtypeByVal,
									 pertrans->transtypeLen);
